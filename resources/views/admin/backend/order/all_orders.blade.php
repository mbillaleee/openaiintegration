                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="tb-col tb-col-md">
                                <div class="fs-6 text-light d-inline-flex flex-wrap gap gx-2">{{ $order->user->name ?? '' }}</div>
                            </td>
                            <td class="tb-col tb-col-sm">
                                <div class="fs-6 text-light">{{ $order->plan->name ?? ''  }} Per month</div>
                            </td>
                            <td class="tb-col tb-col-sm">
                                <div class="badge text-bg-success-soft rounded-pill px-2 py-1 fs-6 lh-sm">{{ \Carbon\Carbon::parse($order->payment_date)->format('Y-m-d')}}</div>
                            </td>
                            <td class="tb-col tb-col-sm">
                                <div class="badge text-bg-success-soft rounded-pill px-2 py-1 fs-6 lh-sm">${{ $order->total }}</div>
                            </td>
                            <td class="tb-col tb-col-sm">
                                <div class="badge text-bg-success-soft rounded-pill px-2 py-1 fs-6 lh-sm">{{ $order->status }}</div>
                            </td>
                            <td class="tb-col tb-col-end">
                                @if($order->status == 'Pending')
                                <a href="{{ route('update.order.status', $order->id) }}" class="btn btn-success btn-sm">Update</a>
                                @else
